...
HEALTH_FONT = pygame.font.SysFont('comicans',40)
...
def yellow_movment(keys_pressed,yellow):
    ...
    if keys_pressed[pygame.K_w]and yellow.y - VELOCITY>=0:  # up
        yellow.y -= VELOCITY
    if keys_pressed[pygame.K_s]and yellow.y + VELOCITY + yellow.height <= HEIGTH:  # down
        yellow.y += VELOCITY
...
def red_movement(keys_pressed,red):
    ...
    if keys_pressed[pygame.K_UP]and red.y - VELOCITY>=0:  # up
        red.y -= VELOCITY
    if keys_pressed[pygame.K_DOWN]and red.y + VELOCITY + red.height <= HEIGTH:  # down
        red.y += VELOCITY

def handle_bullets(yellow_bullets,red_bullets,yellow,red):
    for bullets in yellow_bullets:
        bullets.x += BULLETS_VELOCITY
...
def main():
    ...
    while run:
        ...
        winer_text=""
        if red_health<=0:
            winer_text = "yellow wins"
        if yellow_health<=0:
            winer_text = "red wons"
        if winer_text!="":
            draw_winner(winer_text)
            break
